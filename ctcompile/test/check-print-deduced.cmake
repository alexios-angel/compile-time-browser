...
foreach(line IN LISTS deduced_lines)
  ...
  if(line MATCHES "^[ \t]*auto( const)? [A-Za-z_0-9]+ = ")
...
foreach(line IN LISTS plain_lines)
  if(line MATCHES "^[ \t]*auto( const)? [A-Za-z_0-9]+ = |CTCOMPILE_PIN")
...
function(normalise lines out)
  ...
  foreach(line IN LISTS lines)
    ...
    # but keep const so the explicit and deduced declarations must agree.
    ...
    string(REGEX REPLACE "^([ \t]*)(auto|double|bool|int32_t|int64_t|float|std::string|ctnative::nullable_scalar|ctnative::nullable_string|ctnative::object_value)(( const)? [A-Za-z_0-9]+ = )" "\\1T\\3" line "${line}")
    ...
    string(REGEX REPLACE "^([ \t]*)std::shared_ptr<ctnative::(number_map|map_storage)<([^;=]+)>>(( const)? [A-Za-z_0-9]+ = )" "\\1T\\4" line "${line}")
    string(REGEX REPLACE "^([ \t]*)ctnative::ctn_env_[A-Za-z_0-9]+(( const)? [A-Za-z_0-9]+ = )" "\\1T\\2" line "${line}")
    string(REGEX REPLACE "^([ \t]*)std::shared_ptr<ctnative::method_table_[A-Za-z_0-9]+>(( const)? [A-Za-z_0-9]+ = )" "\\1T\\2" line "${line}")
    string(REGEX REPLACE "^([ \t]*)std::shared_ptr<ctnative::identity_object>(( const)? [A-Za-z_0-9]+ = )" "\\1T\\2" line "${line}")
