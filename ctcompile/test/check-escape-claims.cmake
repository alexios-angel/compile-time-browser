...
if(STRICT)
  ...
  foreach(_line IN LISTS _recording_lines)
    ...
    elseif(_function_name MATCHES "^primitiveMixedMul(Early|Retained|Opaque)$" AND _line MATCHES "^alloc ")
      if(NOT _line MATCHES "^alloc ([0-9]+) kind obj$")
        message(FATAL_ERROR "${_function_name}: unexpected mixed BigInt Mul source allocation: ${_line}")
      endif()
      list(APPEND _primitive_mixed_mul_literal_pcs "${_function_name} ${CMAKE_MATCH_1}")
    elseif(_function_name MATCHES "^primitiveMixedMul(Early|Retained|Opaque)$" AND _line MATCHES "^site ")
      string(REGEX REPLACE "^primitiveMixedMul" "" _mixed_mul_suffix "${_function_name}")
      set(_mixed_mul_error_pc "${_primitive_mixed_mul_error_pc_${_mixed_mul_suffix}}")
      if(NOT _line MATCHES "^site ([0-9]+) kind obj made ([0-9]+) confined ([0-9]+) escaped ([0-9]+) unresolved ([0-9]+) unchecked ([0-9]+) routes ([^ ]+)$")
        message(FATAL_ERROR "${_function_name}: unexpected mixed BigInt Mul observation: ${_line}")
      endif()
      set(_pc "${CMAKE_MATCH_1}")
      set(_row "${_function_name} ${CMAKE_MATCH_2} ${CMAKE_MATCH_3} ${CMAKE_MATCH_4} ${CMAKE_MATCH_5} ${CMAKE_MATCH_6} ${CMAKE_MATCH_7}")
      if(_pc STREQUAL _mixed_mul_error_pc)
        if(NOT _line STREQUAL "site ${_mixed_mul_error_pc} kind obj made 1 confined 0 escaped 1 unresolved 0 unchecked 0 routes thrown:1")
          message(FATAL_ERROR "${_function_name}: the independent mixed Mul Error was not retained through unwinding: ${_line}")
        endif()
        if(_claim_text MATCHES "escape ${_program_hash} ${_function_index} ${_mixed_mul_error_pc} [^\n]+")
          message(FATAL_ERROR "${_function_name}: the implicit mixed Mul Error acquired a source allocation claim")
        endif()
        list(APPEND _primitive_mixed_mul_error_rows "${_row} pc${_pc} unclaimed")
      else()
        string(REGEX MATCHALL "escape ${_program_hash} ${_function_index} ${_pc} obj [^\n]+" _mixed_mul_claims "${_claim_text}")
        list(LENGTH _mixed_mul_claims _mixed_mul_claim_count)
        if(NOT _mixed_mul_claim_count EQUAL 1)
          message(FATAL_ERROR "${_function_name}: missing or duplicate mixed BigInt Mul claim at pc ${_pc}")
        endif()
        if(NOT _claim_text MATCHES "escape ${_program_hash} ${_function_index} ${_pc} obj ([^\n]+)")
          message(FATAL_ERROR "${_function_name}: no compiler claim for observed object at pc ${_pc}")
        endif()
        list(APPEND _primitive_mixed_mul_rows "${_row} ${CMAKE_MATCH_1} pc${_pc}")
      endif()
  ...
  list(SORT _expected_primitive_mixed_mul_literal_pcs)
  list(SORT _primitive_mixed_mul_literal_pcs)
  if(NOT _primitive_mixed_mul_literal_pcs STREQUAL _expected_primitive_mixed_mul_literal_pcs)
    message(FATAL_ERROR "mixed BigInt Mul literal_pcs mismatch:\nexpected: ${_expected_primitive_mixed_mul_literal_pcs}\nobserved: ${_primitive_mixed_mul_literal_pcs}")
  endif()
  ...
  list(SORT _expected_primitive_mixed_mul_rows)
  list(SORT _primitive_mixed_mul_rows)
  if(NOT _primitive_mixed_mul_rows STREQUAL _expected_primitive_mixed_mul_rows)
    message(FATAL_ERROR "mixed BigInt Mul rows mismatch:\nexpected: ${_expected_primitive_mixed_mul_rows}\nobserved: ${_primitive_mixed_mul_rows}")
  endif()
  ...
  list(SORT _expected_primitive_mixed_mul_error_rows)
  list(SORT _primitive_mixed_mul_error_rows)
  if(NOT _primitive_mixed_mul_error_rows STREQUAL _expected_primitive_mixed_mul_error_rows)
    message(FATAL_ERROR "mixed BigInt Mul error_rows mismatch:\nexpected: ${_expected_primitive_mixed_mul_error_rows}\nobserved: ${_primitive_mixed_mul_error_rows}")
  endif()
  message(STATUS "imported mixed BigInt Mul: twelve literal sites, twenty-one instances, ten retained; three independent Errors and live claims agree")
endif()
