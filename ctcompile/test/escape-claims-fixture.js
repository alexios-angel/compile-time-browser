...
function objectFrameBigIntPowSaved(choice) {
    var child = { id: 1 };
    var source = { held: child, base: choice ? -3n : 3n, exponent: 3n };
    var target = { ...source }, base = target.base, exponent = source.exponent;
    target.base = 1; source.exponent = 0;
    delete source.base; delete target.base;
    delete source.exponent; delete target.exponent;
    var power = base ** exponent, squared = power ** 2n, unit = 0n ** 0n;
    delete source.held; delete target.held;
    return { source: source, target: target, power: power, squared: squared, unit: unit };
}
...
function objectFrameBigIntPowPaths(choice) {
    var child = { id: 1 };
    var source = { held: child }, target = { ...source };
    var operand = choice ? 3n : 3, power = operand ** operand;
    delete source.held; delete target.held;
    return { source: source, target: target, power: power };
}
...
function objectFrameBigIntPowMixed(choice) {
    var child = { id: 1 };
    var source = { held: child }, target = { ...source };
    var operand = choice ? -3n : 3n, power = operand ** 3n, equal = power == 27;
    delete source.held; delete target.held;
    return { source: source, target: target, power: power, equal: equal };
}
...
function objectFrameBigIntPowNegativeEarly(choice) {
    var child = { id: 1 };
    var source = { held: child }, target = { ...source };
    var exponent = choice ? -1n : 2n, power = 9n ** exponent;
    delete source.held; delete target.held;
    return { source: source, target: target, power: power };
}
...
function objectFrameBigIntPowNegativeCatch() {
    try { return objectFrameBigIntPowNegativeEarly(true); }
...
function objectFrameBigIntPowCapEarly(choice) {
    var child = { id: 1 };
    var source = { held: child }, target = { ...source };
    var exponent = choice ? 4294967296n : 2n, power = 1n ** exponent;
    delete source.held; delete target.held;
    return { source: source, target: target, power: power };
}
...
function objectFrameBigIntPowCapCatch() {
    try { return objectFrameBigIntPowCapEarly(true); }
    catch (error) { H.push(error); return error; }
}
...
var objectFrameBigIntPowCapErrors = 0, objectFrameBigIntPowCapSuccess = 0;
...
function objectFrameBigIntPowRetained(choice) {
    var child = { id: 1 };
    var source = { held: child }, target = { ...source }, saved = target.held;
    var operand = choice ? -3n : 3n, power = operand ** 3n;
    delete source.held; delete target.held;
    return { source: source, target: target, saved: saved, power: power };
}
