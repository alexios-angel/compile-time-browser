...
with tempfile.TemporaryDirectory(prefix="escape-dump-") as directory:
    ...
    def run(rec, claim):
        ...
        result = subprocess.run(
            [sys.executable, str(checker), "--recording", str(work / "input.rec"),
             "--claims", str(work / "input.claims"), "--dump", str(work / "dump")],
    ...
    implicit_site = next(line + "\n" for line in recording.splitlines() if line.startswith("site 2 "))
    mutations = [
        ("missing allocation", recording.replace("alloc 1 kind obj\n", ""), claims),
        ("extra allocation", recording.replace("alloc 1 kind obj", "alloc 2 kind obj\nalloc 1 kind obj"), claims),
        ("missing observation", recording.replace(source_site, ""), claims),
        ("extra observation", recording.replace(source_site, source_site + implicit_site.replace("site 2", "site 3")), claims),
        ("changed route", recording.replace("globals:1,temporaries:2", "globals:2,temporaries:1"), claims),
        ...
        ("missing unobserved claim", recording, claims.replace("escape abc 0 5 arr confined\n", "")),
    ...
    rejected = [
        ("duplicate allocation", recording.replace("alloc 1 kind obj", "alloc 1 kind obj\nalloc 1 kind obj"), claims),
        ...
        ("duplicate function", recording + "fn 1 entries 0 params 0 frame 0 name duplicate\n", claims),
        ("duplicate program", recording + "program abc size 0 functions 0 label -\n", claims),
        ("duplicate root route", recording.replace("globals:1,temporaries:2", "globals:1,globals:2"), claims),
...
print(f"escape dump: fixture matches; {len(mutations)} evidence mutations and {len(rejected)} malformed/duplicate controls pass")
