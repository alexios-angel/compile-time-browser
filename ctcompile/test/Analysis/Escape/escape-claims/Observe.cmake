  foreach(_line IN LISTS _recording_lines)
    ...
    elseif(_function_name MATCHES "^primitiveUShr(Early|Retained|Opaque)$" AND _line MATCHES "^alloc ")
      if(NOT _line MATCHES "^alloc ([0-9]+) kind (obj|arr)$")
        message(FATAL_ERROR "${_function_name}: unexpected BigInt UShr source allocation: ${_line}")
      endif()
      list(APPEND _primitive_ushr_literal_pcs "${_function_name} ${CMAKE_MATCH_1} ${CMAKE_MATCH_2}")
    elseif(_function_name MATCHES "^primitiveUShr(Early|Retained|Opaque)$" AND _line MATCHES "^site ")
      string(REGEX REPLACE "^primitiveUShr" "" _ushr_suffix "${_function_name}")
      set(_ushr_error_pc "${_primitive_ushr_error_pc_${_ushr_suffix}}")
      if(NOT _line MATCHES "^site ([0-9]+) kind (obj|arr) made ([0-9]+) confined ([0-9]+) escaped ([0-9]+) unresolved ([0-9]+) unchecked ([0-9]+) routes ([^ ]+)$")
        message(FATAL_ERROR "${_function_name}: unexpected BigInt UShr observation: ${_line}")
      endif()
      set(_pc "${CMAKE_MATCH_1}")
      set(_kind "${CMAKE_MATCH_2}")
      set(_row "${_function_name} ${_kind} ${CMAKE_MATCH_3} ${CMAKE_MATCH_4} ${CMAKE_MATCH_5} ${CMAKE_MATCH_6} ${CMAKE_MATCH_7} ${CMAKE_MATCH_8}")
      if(_pc STREQUAL _ushr_error_pc)
        if(NOT _line STREQUAL "site ${_ushr_error_pc} kind obj made 1 confined 0 escaped 1 unresolved 0 unchecked 0 routes thrown:1")
          message(FATAL_ERROR "${_function_name}: the independent UShr Error was not retained through unwinding: ${_line}")
        endif()
        if(_claim_text MATCHES "escape ${_program_hash} ${_function_index} ${_ushr_error_pc} [^\n]+")
          message(FATAL_ERROR "${_function_name}: the implicit UShr Error acquired a source allocation claim")
        endif()
        list(APPEND _primitive_ushr_error_rows "${_row} pc${_pc} unclaimed")
      else()
        string(REGEX MATCHALL "escape ${_program_hash} ${_function_index} ${_pc} ${_kind} [^\n]+" _ushr_claims "${_claim_text}")
        list(LENGTH _ushr_claims _ushr_claim_count)
        if(NOT _ushr_claim_count EQUAL 1)
          message(FATAL_ERROR "${_function_name}: missing or duplicate BigInt UShr claim at pc ${_pc}")
        endif()
        if(NOT _claim_text MATCHES "escape ${_program_hash} ${_function_index} ${_pc} ${_kind} ([^\n]+)")
          message(FATAL_ERROR "${_function_name}: no compiler claim for observed ${_kind} at pc ${_pc}")
        endif()
        list(APPEND _primitive_ushr_rows "${_row} ${CMAKE_MATCH_1} pc${_pc}")
      endif()
