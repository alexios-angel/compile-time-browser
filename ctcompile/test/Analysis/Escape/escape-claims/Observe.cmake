  foreach(_line IN LISTS _recording_lines)
    ...
    elseif(_function_name MATCHES "^primitive(UShr|MixedStatic)(Early|Retained|Opaque)$" AND _line MATCHES "^alloc ")
      string(REGEX MATCH "(UShr|MixedStatic)(Early|Retained|Opaque)$" _static_name "${_function_name}")
      string(TOLOWER "${CMAKE_MATCH_1}" _static_family)
      if(NOT _line MATCHES "^alloc ([0-9]+) kind (obj|arr)$")
        message(FATAL_ERROR "${_function_name}: unexpected static BigInt source allocation: ${_line}")
      endif()
      list(APPEND _primitive_${_static_family}_literal_pcs "${_function_name} ${CMAKE_MATCH_1} ${CMAKE_MATCH_2}")
    elseif(_function_name MATCHES "^primitive(UShr|MixedStatic)(Early|Retained|Opaque)$" AND _line MATCHES "^site ")
      string(REGEX MATCH "(UShr|MixedStatic)(Early|Retained|Opaque)$" _static_name "${_function_name}")
      string(TOLOWER "${CMAKE_MATCH_1}" _static_family)
      ...
      if(_pc STREQUAL _static_error_pc)
        if(NOT _line STREQUAL "site ${_static_error_pc} kind obj made 1 confined 0 escaped 1 unresolved 0 unchecked 0 routes thrown:1")
          message(FATAL_ERROR "${_function_name}: the independent static BigInt Error was not retained through unwinding: ${_line}")
        endif()
        if(_claim_text MATCHES "escape ${_program_hash} ${_function_index} ${_static_error_pc} [^\n]+")
          message(FATAL_ERROR "${_function_name}: the implicit static BigInt Error acquired a source allocation claim")
        endif()
        ...
        if(NOT _static_claim_count EQUAL 1)
          message(FATAL_ERROR "${_function_name}: missing or duplicate static BigInt claim at pc ${_pc}")
        endif()
        if(NOT _claim_text MATCHES "escape ${_program_hash} ${_function_index} ${_pc} ${_kind} ([^\n]+)")
          message(FATAL_ERROR "${_function_name}: no compiler claim for observed ${_kind} at pc ${_pc}")
        endif()
