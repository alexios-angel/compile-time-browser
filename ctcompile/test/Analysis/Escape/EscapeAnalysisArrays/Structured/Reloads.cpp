...
void StructuredCases::reloads() {
    ...
    for (const std::string kind : {"bitand", "bitor"}) {
        ...
        rows.push_back(
            {.what = "unit input strides refine holes left by non-affine masks",
    ...
    for (const std::string kind : {"bitand", "bitor"}) {
        ...
        for (const auto & input :
            ...
            rows.push_back(
                {.what = "crossing low-bit masks retain children outside their actual writes",
    ...
    const auto partitionedShift =
        replace(replace(replace(replace(roundedShift, "[%y, %three, %y, %one, %y, %one]",
                                        "[%y, %two, %y, %one, %one, %one, %one, %one, %one, %y, "
                                        "%one, %y, %one, %one, %one, %one, %one, %one]"),
    ...
    for (const auto & kind : {"shr", "ushr"}) {
        ...
        rows.push_back(
            {.what = "partitioned mixed-shift enclosures preserve disjoint reload gaps",
             .body = body,
             .arrays =
             ...
             .reads = "a[1]=two; a[0]=zero; a[1]=two; a[3]=one; a[1]=two; a[6]=one; a[1]=two; "
                      "a[9]=y; a[1]=two; a[12]=one; a[1]=two; a[15]=one",
             .exit = "a -> {a}"});
        rows.push_back(
            {.what = "partitioned mixed-shift bounds stay aligned to a nonzero start",
             .body = replace(body, "%index = %zero", "%index = %three"),
             .arrays =
                 "a:[zero,two,zero,one,one,one,one,one,one,zero,one,zero,one,one,one,one,one,one]",
             .reads = "a[1]=two; a[3]=one; a[1]=two; a[6]=one; a[1]=two; a[9]=y; a[1]=two; "
                      "a[12]=one; a[1]=two; a[15]=one",
             .exit = "a -> {a}"});
        rows.push_back(
            {.what = "partitioned mixed-shift replay retains an unwritten child",
             .body = replace(body, "%two, %y, %one, %one", "%two, %y, %one, %y"),
             .arrays =
                 "a:[zero,two,zero,one,y,one,one,one,one,zero,one,zero,one,one,one,one,one,one]",
             .reads = "a[1]=two; a[0]=zero; a[1]=two; a[3]=one; a[1]=two; a[6]=one; a[1]=two; "
                      "a[9]=y; a[1]=two; a[12]=one; a[1]=two; a[15]=one",
             .exit = "a -> {a,y}"});
        rows.push_back(
            {.what = "partitioned mixed-shift replay retains a pre-loop snapshot",
             .body = replace(replace(body, "  %finalIndex,",
                                     "  %before = ctjs.get_property %a[%zero]\n  %finalIndex,"),
                             "ctjs.return %a", "ctjs.return %before"),
             .arrays =
                 "a:[zero,two,zero,one,one,one,one,one,one,zero,one,zero,one,one,one,one,one,one]",
             .reads = "a[0]=y; a[1]=two; a[0]=zero; a[1]=two; a[3]=one; a[1]=two; a[6]=one; "
                      "a[1]=two; a[9]=y; a[1]=two; a[12]=one; a[1]=two; a[15]=one",
             .exit = "y -> {y}"});
        reject("partitioned mixed shifts reject an interior count overwrite",
               replace(replace(body, "[%y, %two, %y,", "[%y, %two, %two,"), "%base[%one]",
                       "%base[%two]"));
        reject(
            "partitioned mixed shifts retain the complete later-store census",
            replace(body, "    %step =", "    ctjs.set_property %base[%one], %one\n    %step ="));
    }
    std::string manyShiftElements = "[%y, %two";
    for (unsigned i = 2; i < 360; ++i) { manyShiftElements += i == 2 ? ", %y" : ", %one"; }
    manyShiftElements += "]";
    const auto manyShifts = replace(replace(partitionedShift,
                                            "[%y, %two, %y, %one, %one, %one, %one, %one, %one, "
                                            "%y, %one, %y, %one, %one, %one, %one, %one, %one]",
                                            manyShiftElements),
    ...
    const auto convertedMixedShift = replace(
        replace(replace(replace(roundedShift, "4621256167635550208", "4621819117588971520"),
                        "[%y, %three, %y, %one, %y, %one]", "[%one, %y, %three, %one, %y, %y]"),
                "%base[%one]", "%base[%two]"),
        "%position = ctjs.binary_static shr %scaled, %count",
        "%rounded = ctjs.binary_static shr %scaled, %count\n"
    ...
    for (const auto & body : {convertedMixedShift, convertedLeftShift}) {
        rows.push_back(
            {.what = "composed conversion jumps retain mixed-shift gap refinement",
             .body = body,
    ...
    const auto convertedSignedShift =
        replace(replace(convertedMixedShift, "[%one, %y, %three, %one, %y, %y]",
                        "[%y, %y, %three, %one, %y, %one]"),
    ...
    for (const auto & body : {convertedSignedShift, convertedUnsignedShift}) {
        rows.push_back(
            {.what = "composed right-shift conversion jumps retain mixed-shift gaps",
             .body = body,
             .arrays = "a:[zero,zero,three,one,zero,one]",
             .reads = "a[2]=three; a[0]=y; a[2]=three; a[2]=three; a[2]=three; a[4]=zero",
             .exit = "a -> {a}"});
    }
    rows.push_back({.what = "mixed-shift conversion replay retains an unwritten child",
                    .body = replace(convertedMixedShift, "[%one, %y, %three,", "[%y, %y, %three,"),
                    .arrays = "a:[y,zero,three,one,zero,zero]",
                    .reads = "a[2]=three; a[0]=y; a[2]=three; a[2]=three; a[2]=three; a[4]=zero",
                    .exit = "a -> {a,y}"});
    rows.push_back(
        {.what = "mixed-shift conversion replay retains a pre-loop snapshot",
         .body = replace(replace(convertedMixedShift, "  %finalIndex,",
                                 "  %before = ctjs.get_property %a[%one]\n  %finalIndex,"),
                         "ctjs.return %a", "ctjs.return %before"),
         .arrays = "a:[one,zero,three,one,zero,zero]",
         .reads = "a[1]=y; a[2]=three; a[0]=one; a[2]=three; a[2]=three; a[2]=three; a[4]=zero",
         .exit = "y -> {y}"});
    reject("mixed-shift conversion gaps reject actual count overwrites",
           replace(replace(convertedMixedShift, "[%one, %y, %three,", "[%one, %three, %three,"),
    ...
    rows.push_back({.what = "direct sparse AND masks preserve reload gaps without an earlier shift",
                    .body = directMask,
                    .arrays = "a:[zero,zero,five,one,zero,one,one,one]",
                    .reads = "a[2]=five; a[0]=zero; a[2]=five; a[3]=one; a[2]=five; a[6]=one",
                    .exit = "a -> {a}"});
    rows.push_back(
        {.what = "direct sparse OR masks preserve reload gaps without an earlier shift",
         .body = replace(replace(replace(directMask, "[%y, %y, %five, %one, %y, %one, %one, %one]",
                                         "[%one, %one, %y, %y, %two, %one, %y, %one]"),
                                 "%count = ctjs.get_property %base[%two]",
         ...
         .exit = "a -> {a}"});
    rows.push_back(
        {.what = "direct sparse XOR masks preserve reload gaps without an earlier shift",
         .body = replace(replace(replace(directMask, "[%y, %y, %five, %one, %y, %one, %one, %one]",
                                         "[%one, %y, %y, %two, %y, %one, %one, %one]"),
                                 "%count = ctjs.get_property %base[%two]",
                                 "%count = ctjs.get_property %base[%three]"),
         ...
         .exit = "a -> {a}"});
    rows.push_back(
        {.what = "direct mask replay retains an unwritten child",
         .body = replace(directMask, "[%y, %y, %five, %one, %y,", "[%y, %y, %five, %y, %y,"),
         .arrays = "a:[zero,zero,five,y,zero,one,one,one]",
         .reads = "a[2]=five; a[0]=zero; a[2]=five; a[3]=y; a[2]=five; a[6]=one",
         .exit = "a -> {a,y}"});
    rows.push_back(
        {.what = "direct mask replay retains a saved child after its slot is cleared",
         .body = replace(replace(directMask, "  %finalIndex,",
                                 "  %before = ctjs.get_property %a[%zero]\n  %finalIndex,"),
                         "ctjs.return %a", "ctjs.return %before"),
         .arrays = "a:[zero,zero,five,one,zero,one,one,one]",
         .reads = "a[0]=y; a[2]=five; a[0]=zero; a[2]=five; a[3]=one; a[2]=five; a[6]=one",
         .exit = "y -> {y}"});
    reject("direct mask gap proof rejects an actual mask overwrite",
           replace(replace(directMask, "[%y, %y, %five,", "[%y, %five, %five,"),
                   "%count = ctjs.get_property %base[%two]",
                   "%count = ctjs.get_property %base[%one]"));
    reject(
        "direct mask gap proof retains the complete later-store census",
    ...
    const auto directRemainder = replace(
        replace(replace(replace(directMask, "[%y, %y, %five, %one, %y, %one, %one, %one]",
                                "[%y, %seven, %y, %y, %one, %one, %y, %one, %one, %one]"),
                        "  %a =",
                        "  %seven = ctjs.constant #ctjs.number<4619567317775286272> "
    ...
    for (const auto & expression : {"%position = ctjs.binary mod %i, %count",
                                    ...
                                    "    %position = ctjs.unary neg %part"}) {
        rows.push_back(
            {.what = "direct remainder gaps preserve reloaded divisors for either operand sign",
    ...
    rows.push_back({.what = "direct remainder refinement retains children outside actual writes",
                    .body = replace(directRemainder, "%seven, %y, %y, %one, %one,",
                                    "%seven, %y, %y, %y, %one,"),
                    .arrays = "a:[zero,seven,zero,zero,y,one,zero,one,one,one]",
                    .reads = "a[1]=seven; a[0]=zero; a[1]=seven; a[3]=zero; "
                             "a[1]=seven; a[6]=zero; a[1]=seven; a[9]=one",
                    .exit = "a -> {a,y}"});
    rows.push_back(
        {.what = "direct remainder replay retains a child saved before its overwrite",
         .body = replace(replace(directRemainder, "  %finalIndex,",
                                 "  %before = ctjs.get_property %a[%zero]\n  %finalIndex,"),
                         "ctjs.return %a", "ctjs.return %before"),
         .arrays = "a:[zero,seven,zero,zero,one,one,zero,one,one,one]",
         .reads = "a[0]=y; a[1]=seven; a[0]=zero; a[1]=seven; a[3]=zero; "
                  "a[1]=seven; a[6]=zero; a[1]=seven; a[9]=one",
         .exit = "y -> {y}"});
    reject("direct remainder refinement rejects an actual divisor overwrite",
           replace(replace(directRemainder, "[%y, %seven, %y, %y,", "[%y, %one, %y, %seven,"),
                   "%count = ctjs.get_property %base[%one]",
                   "%count = ctjs.get_property %base[%three]"));
    ...
    const auto crossingMask = replace(
        replace(replace(directConverted, "[%one, %shiftCount, %y, %one, %y, %one, %one, %y]",
                        "[%y, %shiftCount, %one, %y, %one, %one, %y, %one]"),
                "  %a =",
    ...
    rows.push_back(
        {.what = "crossing XOR masks retain bounded reload gaps",
         .body = crossingMask,
         ...
         .exit = "a -> {a}"});
    rows.push_back(
        {.what = "crossing OR masks retain bounded reload gaps",
         .body = replace(replace(crossingMask, "[%y, %shiftCount, %one, %y, %one, %one, %y, %one]",
                                 "[%one, %shiftCount, %one, %y, %one, %y, %one, %y]"),
                         "bitxor %scaled, %sign", "bitor %scaled, %big"),
         .arrays = "a:[one,shiftCount,one,zero,one,zero,one,zero]",
         .reads = "a[1]=shiftCount; a[0]=one; a[1]=shiftCount; a[3]=y; a[1]=shiftCount; a[6]=one",
         .exit = "a -> {a}"});
    rows.push_back(
        {.what = "crossing AND masks retain bounded reload gaps",
         .body = replace(replace(crossingMask, "[%y, %shiftCount, %one, %y, %one, %one, %y, %one]",
                                 "[%one, %shiftCount, %y, %one, %y, %one, %y, %one]"),
         ...
         .exit = "a -> {a}"});
    rows.push_back(
        {.what = "crossing masks also retain gaps across signed zero",
         .body = replace(
             replace(crossingMask, "[%y, %shiftCount, %one, %y, %one, %one, %y, %one]",
                     "[%y, %shiftCount, %one, %y, %one, %y, %one, %one]"),
             "%scaled = ctjs.binary mul %i, %big",
             "%offset = ctjs.binary sub %i, %three\n    %scaled = ctjs.binary mul %offset, %big"),
         .arrays = "a:[zero,shiftCount,one,zero,one,zero,one,one]",
         .reads = "a[1]=shiftCount; a[0]=y; a[1]=shiftCount; a[3]=y; a[1]=shiftCount; a[6]=one",
         .exit = "a -> {a}"});
    rows.push_back(
        {.what = "crossing mask replay retains an unwritten child",
         .body = replace(crossingMask, "%shiftCount, %one, %y,", "%shiftCount, %y, %y,"),
         .arrays = "a:[zero,shiftCount,y,zero,one,one,zero,one]",
         .reads =
             "a[1]=shiftCount; a[0]=zero; a[1]=shiftCount; a[3]=zero; a[1]=shiftCount; a[6]=zero",
         .exit = "a -> {a,y}"});
    rows.push_back(
        {.what = "crossing mask replay preserves a saved child",
         .body = replace(replace(crossingMask, "  %finalIndex,",
                                 "  %before = ctjs.get_property %a[%zero]\n  %finalIndex,"),
                         "ctjs.return %a", "ctjs.return %before"),
         .arrays = "a:[zero,shiftCount,one,zero,one,one,zero,one]",
         .reads = "a[0]=y; a[1]=shiftCount; a[0]=zero; a[1]=shiftCount; a[3]=zero; "
                  "a[1]=shiftCount; a[6]=zero",
         .exit = "y -> {y}"});
    reject(
        "crossing masks reject an actual reloaded mask overwrite",
        replace(replace(replace(crossingMask, "[%y, %shiftCount, %one, %y, %one, %one, %y, %one]",
                                "[%sign, %shiftCount, %one, %y, %one, %one, %y, %one]"),
                        "%masked =", "%mask = ctjs.get_property %base[%zero]\n    %masked ="),
                "bitxor %scaled, %sign", "bitxor %scaled, %mask"));
    reject("crossing mask refinement retains the complete later-store census",
           replace(crossingMask,
                   "    %step =", "    ctjs.set_property %base[%one], %thirty\n    %step ="));
}
