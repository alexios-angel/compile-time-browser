...
void checkSharedMap(mlir::MLIRContext & context) {
  ...
  ctjs.func private @put$4(%this: !ctjs.value, %new: !ctjs.value, %callee: !ctjs.value) -> !ctjs.value attributes {upvalue_count = 1 : i32} {
    ...
    auto objectKey = replaced(capturedFixture, "ctjs.call_direct @make$2(%u, %u, %factory)",
                              "ctjs.call %factory(%u)");
    objectKey =
        replaced(objectKey, "@get$3(%this: !ctjs.value, %new: !ctjs.value, %callee: !ctjs.value)",
                 "@get$3(%this: !ctjs.value, %new: !ctjs.value, %callee: !ctjs.value, "
                 "%entryKey: !ctjs.value)");
    objectKey = replaced(objectKey, "    %answer = ctjs.call %getter(%owned)",
                         "    %actual = ctjs.create_object\n"
                         "    %answer = ctjs.call %getter(%owned, %actual)");
    objectKey = replaced(objectKey,
