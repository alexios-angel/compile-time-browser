...
function counters() {
    ...
    ++k;
    var m = 2147483647;
    m++; // 2147483648: the row must be f64, never i32
    var f = 0.5;
    f++;
    var d = 0;
    d--; // op::sub on proved operands
    for (var i = 0; i < 3; i++) {}
    return [k, m, f, d, i];
}
counters();
...
var nanBuf = new ArrayBuffer(8),
    nanBytes = new Uint8Array(nanBuf),
    nanF64 = new Float64Array(nanBuf);
nanBytes[6] = 0xF4;
nanBytes[7] = 0x7F;
for (var payload = 0; payload < 4; payload++) {
    nanBytes[0] = payload;
    poke(nanF64[0]);
}
