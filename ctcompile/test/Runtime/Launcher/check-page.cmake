...
# DREW.
#
# check-application.cmake beside this does the same for a bare `script::context`.
...
# spelled out in check-application.cmake: both arms run the same game, so a page
...
foreach(required VM AOT WORK)
  if(NOT DEFINED ${required})
    message(FATAL_ERROR "check-page.cmake: -D${required}= is required")
  endif()
endforeach()
...
function(run_arm which binary out_stderr)
  ...
  if(NOT code EQUAL 0)
    message(FATAL_ERROR "check-page.cmake: the ${which} arm exited ${code}\n${complaints}")
  endif()
...
function(counter report name into)
  ...
  if(NOT report MATCHES "transition \"${pattern}\" = ([0-9]+)")
    message(FATAL_ERROR
      "check-page.cmake: no counter named ${name} in this arm's report:\n${report}")
  endif()
...
if(NOT same EQUAL 0)
  message(FATAL_ERROR
    "check-page.cmake: the compiled page does not draw what the interpreted page draws. "
...
if(drawn_bytes LESS 230400)
  message(FATAL_ERROR
    "check-page.cmake: the canvas is ${drawn_bytes} bytes, which is not a 320x240 PPM")
endif()
if(NOT aot_err MATCHES "colours = ([0-9]+)")
  message(FATAL_ERROR "check-page.cmake: the aot arm did not count its colours")
endif()
if(CMAKE_MATCH_1 LESS 3)
  message(FATAL_ERROR
    "check-page.cmake: the canvas has ${CMAKE_MATCH_1} colour(s) on it - the game did "
...
if(NOT aot_err MATCHES "installed ([0-9]+) across ([0-9]+) script")
  message(FATAL_ERROR "check-page.cmake: the aot arm did not report an install:\n${aot_err}")
endif()
set(installed ${CMAKE_MATCH_1})
if(NOT installed EQUAL ${EXPECTED_INSTALLED})
  message(FATAL_ERROR
    "check-page.cmake: ${installed} bodies were installed, expected "
...
foreach(never "C++ -> VM" "VM -> AOT" "AOT -> VM")
  ...
  if(NOT seen EQUAL 0)
    message(FATAL_ERROR
      "check-page.cmake: the aot arm crossed ${never} ${seen} time(s), so the "
...
if(entered LESS 21)
  message(FATAL_ERROR
    "check-page.cmake: C++ entered a compiled body only ${entered} time(s) - the page "
...
if(into_cxx LESS 1)
  message(FATAL_ERROR
    "check-page.cmake: no compiled body called a native - the game cannot have drawn "
    "from compiled code:\n${aot_err}")
endif()
...
foreach(never "C++ -> AOT" "VM -> AOT" "AOT -> AOT" "AOT -> VM")
  ...
  if(NOT seen EQUAL 0)
    message(FATAL_ERROR
      "check-page.cmake: the arm with NO generated code crossed ${never} ${seen} "
...
if(ran_interpreted LESS 21)
  message(FATAL_ERROR
    "check-page.cmake: the interpreted arm entered the interpreter only "
