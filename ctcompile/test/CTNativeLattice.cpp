...
Type parse(mlir::MLIRContext & context, std::string_view text) {
    ...
    if (!parsed) { std::printf("FAILED  could not parse the type `%.*s`\n", int(text.size()), text.data()); }
    return parsed;
}
...
const MeetRow kMeetTable[] = {
    ...
    {"!ctnative.bool", "!ctnative.num<f64>", "!ctnative.variant<!ctnative.bool, !ctnative.num<f64>>",
    ...
    // --- unions -------------------------------------------------------------
    {"!ctnative.bool", "!ctnative.str<utf8>", "!ctnative.variant<!ctnative.bool, !ctnative.str<utf8>>",
...
int main() {
    ...
    check("idempotent, bottom is the identity, boxed absorbs, null is absent",
          lawFailures == 0);
    if (lawFailures != 0) {
            ...
            if (meet(a, b) != meet(b, a)) {
                ...
                if (commuteFailures <= 5) {
                    std::printf("        meet(%s, %s) = %s but meet(%s, %s) = %s\n", show(a).c_str(),
                                show(b).c_str(), show(meet(a, b)).c_str(), show(b).c_str(),
                                show(a).c_str(), show(meet(b, a)).c_str());
                }
            }
        }
    }
    ...
    // --- the StaticTyped interface ------------------------------------------
    {
        ...
        auto native = mlir::UnrealizedConversionCastOp::create(builder, loc, mlir::TypeRange{proved},
                                                              mlir::ValueRange{});
