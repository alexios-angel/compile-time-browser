...
# USAGE
#
#   ctcompile/test/CTNative/Checks/snapshot.sh save <dir> [build]
#   ...make the change, rebuild...
#   ctcompile/test/CTNative/Checks/snapshot.sh compare <dir> [build]
#
#   ctcompile/test/CTNative/Checks/snapshot.sh selftest [build]   # proves both teeth
#
...
cmd_selftest() {
  ...
  # the same commands pipeline.cmake uses and compared with the one in
  # the build tree.
  local translate opt
  ...
  "$cmake_bin" -DTRANSLATE="$translate" -DOPT="$opt" \
        -DSOURCE="$tests/CTNative/Fixtures/ControlFlow/globals.js" \
        -DOUTPUT="$work/again.mlir" \
        -P "$here/pipeline.cmake" >/dev/null 2>&1 || {
