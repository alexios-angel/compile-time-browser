...
# The compilation-unit gate (compilation-unit.cmake) already runs the deduced
...
foreach(line IN LISTS deduced_lines)
  if(line MATCHES "^#ifndef CTCOMPILE_NO_TYPE_PINS")
    # PrintDeduced adds one include immediately before its macro, even when
    # the program already includes type_traits. Remove only that added line;
    # genuine program includes must still compare exactly with the plain file.
    list(LENGTH stripped n_before_macro)
    if(n_before_macro EQUAL 0)
      message(FATAL_ERROR "print-deduced (${NAME}): the pin macro has no preceding include")
    endif()
    list(POP_BACK stripped pin_include)
    if(NOT pin_include STREQUAL "#include <type_traits>")
      message(FATAL_ERROR "print-deduced (${NAME}): expected the pin include immediately before its macro, got: ${pin_include}")
    endif()
    set(in_macro ON)
  endif()
  if(in_macro)
  ...
  if(line MATCHES "^[ \t]*(constexpr )?auto( const)? [A-Za-z_0-9]+ = ")
...
foreach(line IN LISTS plain_lines)
  if(line MATCHES "^[ \t]*(constexpr )?auto( const)? [A-Za-z_0-9]+ = |CTCOMPILE_PIN")
...
function(normalise lines out)
  ...
  foreach(line IN LISTS lines)
    # Qualification is part of the contract: normalize the carrier spelling,
