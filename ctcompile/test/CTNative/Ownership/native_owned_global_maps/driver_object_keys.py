...
def check_object_argument_observations(args, node, reference):
    ...
    for name, row in cases.items():
        ...
        for index, replacement in enumerate((f't.set({key}, {{}});', f't.set({key}, 1);',
                                              f't.has({key});', 't.set(2, e);')):
    ...
    return dict(sources=len(cases), observations=len(cases) + 7 + payload_observations,
                mutations=len(mutations) + len(retained_mutations) + len(named_mutations)
                + alias_mutations + payload_mutations + 1)
...
def check_object_argument_preparation(text, original, name):
    ...
    original_calls = re.findall(r'^\s*(%[-\w.$]+) = ctjs\.call %[-\w.$]+\('
                                r'%[-\w.$]+, (%[-\w.$]+)\)', original_entry, re.M)
    receivers = dict(re.findall(r'(%[-\w.$]+) = ctjs\.get_property (%[-\w.$]+)\[', entry))
    captures = dict(re.findall(r'(%[-\w.$]+) = ctjs\.load_upvalue (%[-\w.$]+)\[0\]', entry))
    if not calls or len(calls) != len(original_calls) or len(calls) != row['source'].count('host.slot.get('):
    ...
    def origins(body, actuals):
        ...
        values.update({value: ('global', index, binding)
                       for index, (value, binding) in enumerate(loads)})
        values.update({value: ('constant', literal.strip()) for value, literal in re.findall(
            r'(%[-\w.$]+) = ctjs\.constant ([^\n{]+)', body)})
    ...
    if (origins(entry, arguments) != origins(original_entry, [value for _, value in original_calls])
