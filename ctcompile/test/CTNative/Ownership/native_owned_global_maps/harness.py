...
def standalone(args, output, name, value, compilers, nm):
    ...
    for mode, ir in (("explicit", output), ("deduced", deduced)):
        ...
        size_signature = (
            "std::function<ctnative::js_num(ctnative::nullable_string)>"
            if name in nullable_host_result_sources()
            else (
                "std::function<ctnative::js_boolean_t()>"
                if name == "boolean_result"
                else "std::function<ctnative::js_num()>"
            )
        )
        map_action = (
            "ctnative::map_has("
            if name == "boolean_result" or (object_argument and name != "parameter_object")
            else "ctnative::map_size("
        )
        if (
            owned.VM.search(cpp)
            or "std::shared_ptr<ctn_slot>" not in cpp
            or (not object_argument and size_signature not in cpp)
            or map_action not in cpp
            or not re.search(r"std::shared_ptr<ctnative::method_\w+>\s+slot\s*;", cpp)
        ):
            raise RuntimeError(
                f"{name}/{mode}: missing standalone Map/table/callable owners\n{cpp}"
            )
        if object_argument:
        ...
        if name == "boolean_result":
            ...
            if (
                not entry
                or not getter
                or entry[1].count("ctnative::invoke_callable(") != 1
                ...
                raise RuntimeError(
                    f"{name}/{mode}: lost the live Boolean result or its exact observation tag"
                )
        ...
        if name in RESULT_SIGNATURES:
            ...
            setter_result = (
                ...
                if name in NULLABLE_PAYLOAD_READBACKS
            ...
            if (
                ...
                raise RuntimeError(
                    f"{name}/{mode}: missing typed producer/consumer signatures\n{cpp}"
                )
        ...
        if name in primitive_absence_sources() and (
            "std::shared_ptr<ctnative::map_storage<ctnative::nullable_string, std::string>>"
            not in cpp
        ):
            raise RuntimeError(
                f"{name}/{mode}: lost the independent nullable-key/String-payload carrier"
            )
        if name in MIXED_RESULT_TYPES:
            ...
            if not any(
                f"std::shared_ptr<ctnative::map_storage<{k}, {v}>>" in cpp
                for k in spellings
                for v in {variant, variant.replace("double", "js_num")}
            ):
                raise RuntimeError(
                    f"{name}/{mode}: missing exact finite key/payload carrier\n{cpp}"
                )
        ...
        if name in {"joined_size_saved_lifetime", "joined_mutation_saved_lifetime"}:
            ...
            source.write_text(zero_size_lifetime_cpp(cpp))
        if name in {
            "object_argument_exact",
            "object_argument_global",
        ...
        if name in leaf_object_sources() and (
            not name.endswith("_repair") or name == "leaf_object_identity_repair"
        ):
            source = args.work / f"{name}.{mode}.identity.cpp"
        ...
        for index, compiler in enumerate(compilers):
            ...
            if (
                normalized_scalar_output(host.run([str(binary)]).stdout)
                != scalar_global_output(name, value) * traces
            ):
