"""sources globals: continued from sources_numeric_globals."""

from .sources_numeric_globals import *

SCALAR_GLOBAL_UNOWNED = {
    "scalar_read_before_write",
    "scalar_later_bool",
    "scalar_future_bool",
    "scalar_duplicate_number",
}
...
def scalar_global_cases():
    ...
    add(
        "scalar_alias",
        snapshot.replace(
            "var trace = first * 10 + second;",
            "const alias = first; var trace = alias * 10 + second;",
        ),
        12,
    ...
    add(
        ...
        snapshot.replace(
            "var trace = first * 10 + second;",
            "const total = first * 10 + second; var trace = total + 1;",
        ),
        13,
    ...
    add(
        "scalar_builtin_spelling",
        snapshot.replace("first", "Reflect").replace("second", "prototype"),
        12,
        {"Reflect": 1, "prototype": 2},
    ...
    add(
        "scalar_result_key",
        prefix + "const first = host.slot.set(1); const second = host.slot.set(2); "
    ...
    add(
        "scalar_duplicate_write",
        single.replace("host.slot.set('y');", "first = host.slot.set('y');"),
        2,
        {"first": 2},
        "a later second write invalidates the single-store proof",
        repair=("first = host.slot.set('y');", "host.slot.set('y');", "scalar_single_write_repair"),
    )
    ...
    add(
        ...
        "NaN",
        {"first": 2},
        "a later declaration and result store cannot authorize an earlier Undefined read",
        repair=(prior, ordered, "scalar_prior_store_repair"),
    )
    field = historical["local_numeric_branch_lifetime"]["source"]
    expression = (
        "var trace = host.slot.set('x', 2, false) + "
        "host.slot.set('y', 3, false) * host.slot.set('z', 4, false);"
    )
    saved = (
        "const first = host.slot.set('x', 2, false); const second = host.slot.set('y', 3, false); "
    ...
    add(
        ...
        field + "host.slot.set('later', 5, false);\n",
        14,
        values,
    ...
    add(
        "scalar_later_bool",
        field + "host.slot.set('later', false, false);\n",
        14,
        values,
        "later mixed actuals invalidate provisional Number result dependencies",
        repair=(
            "host.slot.set('later', false, false);",
            "host.slot.set('later', 5, false);",
            "scalar_later_number_repair",
        ),
    )
    add(
        "scalar_future_number_repair",
        field.replace("return saved.value;", "return value ? saved.value : 0;"),
        14,
        values,
    ...
    add(
        "scalar_future_bool",
        field.replace("return saved.value;", "return value ? saved.value : false;"),
        14,
        values,
        "truthy startup inputs cannot hide a future Boolean public result",
        repair=(
    ...
    add(
        "scalar_constant_literal_repair",
        literal,
        12,
    ...
    add(
        ...
        literal.replace("const copy = 7;", "const copy = fixed;"),
        12,
        {"first": 1, "second": 2, "fixed": 7, "copy": 7},
        "a constant-only Number global has no completed published-result dependency",
        repair=("const copy = fixed;", "const copy = 7;", "scalar_constant_literal_repair"),
    )
    ...
    add(
        "scalar_alias_number_repair",
        alias.replace("const alias = first;", "const alias = first + 0;"),
        12,
        rows["scalar_alias"]["saved"],
        "retain the scalar alias store/read and use definite Number arithmetic at its initializer",
    )
    rows["scalar_alias"].update(
        repair="scalar_alias_number_repair",
        removed_text="const alias = first;",
        replacement_text="const alias = first + 0;",
    )
    add(
        "scalar_single_number_repair",
        single.replace("var trace = first;", "var trace = first + 0;"),
        1,
        {"first": 1},
        "retain the saved global read and both calls with a definite Number observation",
    )
    rows["scalar_single_write_repair"].update(
        repair="scalar_single_number_repair",
        removed_text="var trace = first;",
        replacement_text="var trace = first + 0;",
    )
    rows["scalar_duplicate_write"].update(
        repair="scalar_single_number_repair",
        removed_text="first = host.slot.set('y'); var trace = first;",
        replacement_text="host.slot.set('y'); var trace = first + 0;",
    )
    duplicate = rows["scalar_duplicate_write"]["source"].replace(
        "var trace = first;", "var trace = first + 0;"
    )
    add(
        "scalar_duplicate_number",
        duplicate,
    ...
    add(
        "scalar_alias_chain",
        alias.replace("const alias = first;", "const saved = first; const alias = saved;"),
        12,
    ...
    add(
        "scalar_alias_arithmetic",
        arithmetic.replace("var trace = total + 1;", "const alias = total; var trace = alias + 1;"),
        13,
    ...
    add(
        ...
        branch.replace(
            "var trace = first + second * third;",
            "const left = first; const middle = second; const right = third; "
            "var trace = left + middle * right;",
        ),
        14,
...
def scalar_global_refusals():
    return {
        ...
        for name, row in scalar_global_cases().items()
        if name in SCALAR_GLOBAL_UNOWNED
    }


def scalar_global_values(name):
    saved = {
        "local_add_saved_results": {"first": 1, "second": 1, "third": 1},
        "local_numeric_saved_snapshot": {"first": 1, "second": 2},
    }.get(name)
    if saved is None:
...
STRING_GLOBAL_BYTES = StringValue(
...
CONSTANT_GLOBAL_HISTORY = {
    ...
    "constant_feeds_trace": (
        "032a7cf898ad6677b54369d302038c28ec94083906264e97955f5e66c7b413d1",
        "11bc2b77e812824764f7dc793f1f7c19f2a6bf6d2d1e63a596f6b209f21dcec4",
    ),
    "constant_negative_zero": (
        "fd18ed89f0d6ab85b761908e77708383fbcb51bc79bd942a7ad897217a798f64",
        "aa78e8ee9040963d8781a3d8f06352d7f568f326bcee4a76e45264f1737db605",
    ),
    "constant_nan": (
        "16e8e9f5790f88eef330cfa7992101f54b3a7ad32cfdf0fb2fb24c58566627e1",
    ...
    "constant_undefined": (
        "004faccaffe587297d5df60e77bb78c19a7b21dffe674cd38e8ad2244e050d6a",
        "def9220fcc5c8516122b8e1792454d97a505912b94713e6d709b25fb70bc94c6",
    ),
    "constant_builtin_spelling": (
        "804006fadbd519e84980ccdfac5372d268a70bb2f96e76bd3727ad48458b6439",
        "c733a5cda331467b78b20b06c9d9748143905c39ef1b73f1c714a0c8a08ac222",
    ),
    "constant_duplicate_write": (
        "41bd3d6a3f2635099f23de52e6d65c1299be65962b70e8684c19aaccc525da2f",
        "1e93521a3cfda469ebc5b0635a1f5f703db851afbe4ee6cd2244bf3acdaf0fd7",
    ),
    "constant_read_before_write": (
        "37cfd8d9934dd7ba9ebde1fd4eabeb095746bb10299da0551efe7671f3dd698c",
        "69179658e296b40c839a5381005a3e96a3e15a7bbf85216926d2803456780409",
    ),
    "constant_dynamic_global": (
        "37af242bbce92d2ed2e2bee75608b1c703bc93c1660f5e9a0070760f43b60214",
        "1e93521a3cfda469ebc5b0635a1f5f703db851afbe4ee6cd2244bf3acdaf0fd7",
    ),
    "constant_future_method_write": (
        "c456bc57bd87cd1a850afae05dc4bc46d35e234dc6e68d4094800eeaee482375",
...
def constant_global_cases():
    ...
    def add(name, text, trace, saved, old=None, replacement=None, candidate_saved=None):
        calls = 2 + len(
            re.findall(r"\b(?:state|host\.slot)\.(?:set|get|size|has|delete|clear)\(", text)
        )
        rows[name] = dict(
            source=text,
    ...
    add("constant_exact_historical", source, 12, values, "const copy = fixed;", "const copy = 7;")
    prefix = snapshot.replace("var trace = first * 10 + second;\n", "")
    add(
        "constant_feeds_trace",
        prefix + "const fixed = 7; const copy = fixed; var trace = copy + first;\n",
        8,
        values,
        "const copy = fixed;",
        "const copy = 7;",
    ...
    add(
        "constant_negative_zero",
        prefix + "const fixed = 0 / (0 - 1); const copy = fixed; var trace = 1 / copy;\n",
        "-Infinity",
        {**values, "fixed": "-0", "copy": "-0"},
        "const copy = fixed;",
        "const copy = 0 / (0 - 1);",
    )
    add(
        "constant_nan",
        prefix + "const fixed = 0 / 0; const copy = fixed; var trace = copy;\n",
        "NaN",
        {**values, "fixed": "NaN", "copy": "NaN"},
        "const copy = fixed;",
        "const copy = 0 / 0;",
    )
    for tag, literal, value in (
        ...
        add(
            "constant_" + tag,
            source.replace("const fixed = 7;", "const fixed = " + literal + ";"),
            12,
            {**values, "fixed": value, "copy": value},
            "const copy = fixed;",
            "const copy = " + literal + ";",
        )
    add(
        "constant_builtin_spelling",
        source.replace("fixed", "Reflect").replace("copy", "prototype"),
        12,
        {"first": 1, "second": 2, "Reflect": 7, "prototype": 7},
        "const prototype = Reflect;",
        "const prototype = 7;",
    )
    add(
        "constant_duplicate_write",
        snapshot + "var fixed = 7; const copy = fixed; fixed = 9;\n",
        12,
        {**values, "fixed": 9},
        "fixed = 9;",
        "const later = 9;",
    ...
    add(
        "constant_read_before_write",
        snapshot + "var copy = fixed; var fixed = 7;\n",
        12,
        {**values, "copy": "undefined"},
        "var copy = fixed; var fixed = 7;",
        "var fixed = 7; var copy = fixed;",
        values,
    )
    add(
        "constant_dynamic_global",
        snapshot + "var fixed = 7; const copy = fixed; globalThis.fixed = 9;\n",
        12,
        {**values, "fixed": 9},
        "globalThis.fixed = 9;",
        "const later = 9;",
    ...
    add(
        "constant_future_method_write",
        future + "const copy = fixed;\n",
        12,
        {**values, "fixed": 9, "copy": 9},
        "fixed = 9;",
        "const future = 9;",
        values,
    )
    add(
        "constant_alias_chain",
        source.replace("const copy = fixed;", "const offset = fixed; const copy = offset;"),
        12,
        {**values, "offset": 7},
    )
    add(
        "constant_alias_arithmetic",
        prefix + "const fixed = 2 * 3 + 1; const offset = fixed / 2; const copy = offset; "
        "var trace = copy + first;\n",
        4.5,
        {**values, "offset": 3.5, "copy": 3.5},
    )
    ...
    add(
        ...
        12,
        {**flags, "enabled": True, "offset": False},
    )
    add(
        "constant_boolean_builtin_spelling",
        boolean.replace("fixed", "Reflect").replace("copy", "prototype"),
        12,
        {"first": 1, "second": 2, "Reflect": False, "prototype": False},
    )
    for flag, value in (("false", False), ("true", True)):
        add(
            "constant_boolean_trace_" + flag,
            prefix + "const fixed = " + flag + "; const copy = fixed; var trace = copy;\n",
            value,
            {**flags, "fixed": value, "copy": value},
        )
    result = (
        boolean.replace("return state.size; }\n", "return saved === item; }\n")
        .replace("var trace = first * 10 + second;", "var trace = 12;")
        .replace("const copy = fixed;", "const copy = first;")
    )
    ...
    add(
        "constant_boolean_duplicate_write",
        snapshot + "var fixed = false; const copy = fixed; fixed = true;\n",
        12,
        {**flags, "fixed": True},
        "fixed = true;",
        "const later = true;",
    ...
    add(
        "constant_boolean_mixed_write",
        snapshot + "var fixed = false; const copy = fixed; fixed = 1;\n",
        12,
        {**flags, "fixed": 1},
        "fixed = 1;",
        "const later = 1;",
    ...
    add(
        "constant_boolean_read_before_write",
        snapshot + "var copy = fixed; var fixed = false;\n",
        12,
        {**flags, "copy": "undefined"},
        "var copy = fixed; var fixed = false;",
        "var fixed = false; var copy = fixed;",
        flags,
    )
    add(
        "constant_boolean_dynamic_global",
        snapshot + "var fixed = false; const copy = fixed; globalThis.fixed = true;\n",
        12,
        {**flags, "fixed": True},
        "globalThis.fixed = true;",
        "const later = true;",
        {**flags, "later": True},
    )
    ...
    for kind, other in (("optional", "void 0"), ("mixed", "0")):
        ...
        add(
            "constant_boolean_" + kind,
            boolean.replace("const fixed = false;", initializer),
            12,
            flags,
            initializer,
            "const fixed = false;",
    ...
    add(
        ...
        lifetime["source"] + "const fixed_flag = false; const enabled = true; "
    ...
    for kind, value in (
        ("empty", StringValue("")),
        ("bytes", STRING_GLOBAL_BYTES),
        ("long", STRING_GLOBAL_LONG),
    ):
        literal = json.dumps(value, ensure_ascii=False)
        add(
            "constant_string_" + kind,
            string.replace("'owned scalar'", literal),
            12,
            {**strings, "fixed": value, "copy": value},
            "const copy = fixed;",
            "const copy = " + literal + ";",
        )
    add(
        "constant_string_alias_chain",
        string.replace("const copy = fixed;", "const offset = fixed; const copy = offset;"),
        12,
        {**strings, "offset": StringValue("owned scalar")},
    )
    for kind, value in (("empty", StringValue("")), ("bytes", STRING_GLOBAL_BYTES)):
        add(
            "constant_string_trace_" + kind,
            prefix
            + "const fixed = "
            + json.dumps(value, ensure_ascii=False)
            + "; const copy = fixed; var trace = copy;\n",
            value,
            {**strings, "fixed": value, "copy": value},
        )
    add(
        "constant_string_duplicate_write",
        snapshot + "var fixed = 'owned scalar'; const copy = fixed; fixed = 'later';\n",
        12,
        {**strings, "fixed": StringValue("later")},
        "fixed = 'later';",
        "const later = 'later';",
    ...
    add(
        "constant_string_mixed_write",
        snapshot + "var fixed = 'owned scalar'; const copy = fixed; fixed = false;\n",
        12,
        {**strings, "fixed": False},
        "fixed = false;",
        "const later = false;",
    ...
    add(
        "constant_string_read_before_write",
        snapshot + "var copy = fixed; var fixed = 'owned scalar';\n",
        12,
        {**strings, "copy": "undefined"},
        "var copy = fixed; var fixed = 'owned scalar';",
        "var fixed = 'owned scalar'; var copy = fixed;",
        strings,
    )
    future = "var fixed = 'owned scalar';\n" + snapshot.replace(
        "set(key) {", "set(key) { fixed = 'later';"
    )
    add(
        "constant_string_future_method_write",
        future + "const copy = fixed;\n",
    ...
    for kind, other in (("optional", "void 0"), ("mixed", "false")):
        ...
        add(
            "constant_string_" + kind,
            string.replace("const fixed = 'owned scalar';", initializer),
            12,
            strings,
            initializer,
            "const fixed = 'owned scalar';",
        )
    add(
        "constant_string_branch_lifetime",
        lifetime["source"]
        + "const owned_text = "
        + json.dumps(STRING_GLOBAL_LONG)
        + "; const text_alias = owned_text; const saved_text = text_alias; const empty_text = '';\n",
        14,
        {
            **lifetime["saved"],
            "owned_text": STRING_GLOBAL_LONG,
            "text_alias": STRING_GLOBAL_LONG,
            "saved_text": STRING_GLOBAL_LONG,
            "empty_text": StringValue(""),
        },
    )
...
def constant_global_sources():
    return {
        ...
        if name
        not in CONSTANT_GLOBAL_UNOWNED | CONSTANT_GLOBAL_CARRIERS | CONSTANT_GLOBAL_EXISTING.keys()
    }
