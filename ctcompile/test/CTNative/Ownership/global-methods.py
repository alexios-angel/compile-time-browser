...
def resolve_getter(args, ir):
    ...
    text, count = re.subn(
        r"ctjs\.call (%[-\w.$]+)\((%[-\w.$]+)\)",
        lambda call: f"ctjs.call_direct @{target}({call[2]}, {undefined[1]}, {call[1]})",
        text,
    )
    if count != 1:
...
def census(ir, denominator, name, *, admitted=None):
    ...
    if remaining + native != denominator or len(boundary.REFUSAL.findall(text)) != remaining:
        raise RuntimeError(f"{name}: changed source function accounting\n{text}")
    if admitted is not None and native != admitted:
        raise RuntimeError(
            f"{name}: expected {admitted}/{denominator} native, got {native}\n{text}"
        )
    return text
...
def standalone(args, output, name, expected, compilers, nm, *, result_type="js_num"):
    ...
    for mode, ir in (("explicit", output), ("deduced", deduced)):
        ...
        if (
            owned.VM.search(cpp)
            or re.search(r"#include [<\"]ctbrowser/", cpp)
        ...
        if result_type == "ctnative::nullable_scalar" and (
            "ctnative::print_scalar(" not in cpp
            or "ctnative::invoke_callable(" not in cpp
...
def refusal_sources():
    ...
    return {
        ...
        "capture": SOURCE.replace(
            "function make() {", "function make() { const value = 42;"
...
def main():
    ...
    for name, (source, binding, expected) in POSITIVES.items():
        ...
        node_driver = (
            STRING_NODE if string_result else BOOLEAN_NODE if boolean_result else boundary.NODE
        )
    ...
    for budget in range(480, 545):
        ...
        if "ctnative.host_owner_proved = true" in text:
            ...
            for op in ("create_object", "set_property", "get_property", "call", "call_direct"):
                pattern = rf"^\s*(?:%[^=\n]+\s*=\s*)?ctjs\.{op}\b"
                if len(re.findall(pattern, ir.read_text(), re.M)) != len(
    ...
    for name, literal in (("null", "null"), ("undefined", "void 0")):
        js, typed, count = boundary.prepare(
            args, f"typed-{name}", SOURCE.replace("return 42;", f"return {literal};")
        )
        if count != 3:
        ...
        if (
            ...
            or f"1 globals printed ({tags})" not in interpreted.stderr
        ):
    ...
    text, count = re.subn(
        rf"#ctjs\.number<{original_bits}>", f"#ctjs.number<{changed_bits}>", ir.read_text()
    )
    if count != 1:
    ...
    first = refused(args, forged, "forged-stale", config, reason="fingerprint mismatch", admitted=1)
    refused(args, first, "forged-stale-rerun", config, reason="fingerprint mismatch", admitted=1)
    ...
    for name, source in sources.items():
        ...
        if name == "effect":
            ...
            marker = (
                ' {ctnative.owned_method_table_slot = "forged", '
                'ctnative.method_table = "forged"}'
            )
            text, count = re.subn(
                r"\bctjs\.create_object(?!\s*\{)", "ctjs.create_object" + marker, text
            )
            if count != 2:
    ...
    print(
        f"native owned global methods: {len(POSITIVES) + 2} complete 3/3 programs; "
