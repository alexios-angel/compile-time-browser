...
function forwardTable(table) {
    return table;
}

function readTable(table, delta) {
...
function makeLabeled(seed, label) {
    ...
    return {
        get(delta) {
            return state.get("value") + delta + (label === "aa" ? 2 : 4);
        },
        set(value) {
            state.set("value", value);
            return state.get("value") + 0;
...
function readLabeled(table, delta) {
    return table.get(delta);
}
...
function makeSignatures(seed) {
    ...
    return {
        ...
        read(delta) {
            return state.get("key") + seed + delta;
        },
        name(value) {
            return "x" + value;
        }
