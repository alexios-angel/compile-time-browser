...
METHODS = {
    ...
    "diagnose": (
        "",
        "console.error(" + LABEL + " + Array.from(resource.keys())[0] + '.'); return 0;",
    ),
    "next": ("", "return 99;"),
    "wrong": ("", "return -1;"),
}
RECORDER = (
    "callCount = callCount + 1; lastMatch = message === " + json.dumps(MESSAGE) + " ? 41 : 99;"
)
...
class Case:
    ...
    def program(self):
        ...
        lines = [
            self.before,
            "var host = {}; var alias; var iterator;",
...
def cases():
    ...
    result = [
        ...
        Case(
            "repeated_callback",
            [store, diagnose, diagnose, call("host.slot.read('alpha')", 1)],
            final_count=2,
            final_match=41,
            callback_mode=True,
            callbacks=[
                ("recorder", {"callCount": 1, "lastMatch": 41}),
                ("recorder", {"callCount": 2, "lastMatch": 41}),
            ],
        ),
        Case(
            "callback_read_after_write",
            [store, diagnose, call("host.slot.read('alpha')", 1)],
        ...
        Case(
            ...
            [
                ...
                diagnose,
                statement(
                    "console.error = function replacement(message) { "
                    "callCount = callCount + 10; lastMatch = message === "
                    + json.dumps(MESSAGE)
                    + " ? 42 : 99; };"
                ),
                diagnose,
                call("host.slot.read('alpha')", 1),
            ],
            final_count=11,
            final_match=42,
            callback_mode=True,
        ...
        Case(
            "empty_snapshot",
            [call("host.slot.first()", mutations.UNDEFINED)],
            completed=0,
            boundary="first",
        ),
        Case(
            "out_of_range",
            [store, call("host.slot.second()", mutations.UNDEFINED)],
            completed=1,
            boundary="second",
        ),
        Case(
            "fractional_index",
            [store, call("host.slot.fractional()", mutations.UNDEFINED)],
            completed=1,
            boundary="fractional",
        ),
        Case(
            "non_string_key",
            [call("host.slot.store(1, 7)", 0), call("host.slot.first()", 1)],
            completed=1,
            boundary="first",
        ),
        Case(
            "snapshot_escape",
            [store, call("host.slot.bad()", mutations.OBJECT)],
        ...
        Case(
            "snapshot_write",
            [store, call("host.slot.bad()", "changed")],
            methods={
                "bad": (
                    "",
                    "const snapshot = Array.from(resource.keys()); "
        ...
        Case(
            "iterator_mutation",
            [store, call("host.slot.bad()", "late")],
            methods={
                "bad": (
                    "",
                    "const keys = resource.keys(); resource.set('late', 2); "
        ...
        Case(
            "iterator_reuse",
            [store, call("host.slot.bad()", mutations.UNDEFINED)],
            methods={
                "bad": (
                    "",
                    "const keys = resource.keys(); Array.from(keys); "
        ...
        Case(
            "array_from_replaced",
            [store, call("host.slot.first()", "custom")],
            before="Array.from = function() { return ['custom']; };",
            invalid=True,
        ),
        Case(
            "map_keys_replaced",
            [store, call("host.slot.first()", "custom")],
            before="Map.prototype.keys = function() { return ['custom']; };",
            invalid=True,
        ),
        Case(
            "object_coercion",
            [store, call("host.slot.bad()", "Map: [object Map]")],
            methods={"bad": ("", "return 'Map: ' + resource;")},
            completed=1,
            boundary="bad",
        ),
    ]
    refused_callbacks = {
        ...
        "reentrant_callback": (
            RECORDER + " host.slot.erase('alpha'); host.slot.store('alpha', 7);",
            1,
            41,
            None,
        ),
    ...
    result.append(
        Case(
            "console_getter",
            [store, diagnose],
            console="{get error() { host.slot = host.slot; return function recorder(message) { "
            + RECORDER
            + " }; }}",
            completed=0,
            calls=0,
...
def oracle(node, folder, case, js):
    ...
    if observed["observations"] != expected:
        raise RuntimeError(
            f"{case.name}: Node observations {observed['observations']} != {expected}"
        )
...
def interpreter(reference, folder, case, js):
    ...
    evidence = {
        "checked": True,
        "agrees_with_node": case.name not in INTERPRETER_DIVERGENCES,
...
def check_report(case, report, before, after):
    ...
    if (
        case.name == "diagnostic_arguments_only"
        and "`ctjs.call`" not in report["provider_boundary"]
    ):
        raise RuntimeError("diagnostic argument proof did not reach the actual callback call")
    if case.name == "iterator_override" and (
        report["summarized_factories"]
        or report["capture_edges"]
        or report["provider_boundary"]
        or report["boundary"]
        != "resolved call body remains a runtime effect boundary at `ctjs.call`"
    ):
    ...
    if report["selected_branches"] or report["provider_reads"]:
        raise RuntimeError(
            f"{case.name}: diagnostics changed branches or consulted old empty state"
        )
    callbacks = [(row, callback) for row in rows for callback in row["callbacks"]]
    if (report["runtime_provider_callbacks"], len(callbacks)) != (
        len(case.callbacks),
        len(case.callbacks),
    ):
    ...
    for (row, callback), (target, expected) in zip(callbacks, case.callbacks):
        if not callback["target"].startswith(target + "$"):
            raise RuntimeError(
                f"{case.name}: callback retained a stale source identity: {callback}"
            )
        ...
        observed = {
            write["binding"]: mutations.attribute(write["value"]) for write in callback["writes"]
        }
        ...
        if callback["call_operation"] < 0 or row["call_operation"] < 0:
            raise RuntimeError(
                f"{case.name}: callback lost its source call or enclosing invocation"
            )
    if len(callbacks) > 1 and len({row["call_operation"] for row, _ in callbacks}) != len(
        callbacks
    ):
...
def controls(args, prepared):
    ...
    for label, options in [("legacy", BASE), ("arguments-only", DIAGNOSTICS)]:
        report, _, _ = mutations.specialize(
            args.opt, original, manifest, args.work / label, options=options
        )
        if (
    ...
    for label, options in [
        (
            "no-mutations",
            "follow-publication=true follow-provider-reads=true follow-provider-diagnostics=true",
        ),
    ...
    for intrinsic in ("Map", "Array"):
        limited, _, _ = mutations.specialize(
            args.opt,
            original,
            dict(
                manifest,
                initial_intrinsics=[name for name in ("Map", "Array") if name != intrinsic],
            ),
            args.work / f"no-{intrinsic}",
            options=CALLBACKS,
        )
    ...
    _, rewritten, _ = mutations.specialize(
        args.opt, original, manifest, args.work / "once", options=CALLBACKS
    )
    stale, _, _ = mutations.specialize(
        args.opt, rewritten, manifest, args.work / "stale", options=CALLBACKS, success=False
    )
    ...
    forged.write_text(
        unsafe.read_text().replace(
            ...
            "ctnative.host_global_writes = 99 : i64, ctnative.host_snapshots = true, ",
            1,
        )
    )
    report, _, _ = mutations.specialize(
        args.opt, forged, contract(args.opt, unsafe), args.work / "forged-check", options=CALLBACKS
    )
    if (
        report["summarized_provider_calls"] != 1
        or report["runtime_provider_callbacks"]
    ...
    def run_budget(limit):
        ...
        if not report["valid"]:
            ...
            result.returncode
            or report["summarized_provider_calls"] != 4
            or report["runtime_provider_callbacks"] != 2
        ):
...
def main():
    ...
    mode = (
        "Node source cases"
        if args.oracle_only
        else "source cases, runtime bodies, callbacks and rollback controls"
    )
