...
NESTED = {
    "nest": (
        "key, value",
        "const inner = new Map; inner.set('payload', value); "
...
class Case:
    ...
    def expected(self):
        result = {
            f"trace{index}": step.expected
            for index, step in enumerate(step for step in self.steps if step.expected is not None)
        }
    ...
    def program(self):
        ...
        lines = [
            "var host = {}; var first; var alias; var keyA = {}; var keyB = {};",
            f"var objectA = {self.first_object}; var objectB = {self.second_object};",
            declarations,
        ]
        if self.callback is not None:
            lines.append(
                "var console = {error: function recorder(message) { " + self.callback + " }};"
            )
        lines.extend(
            [
                "(function(factory) { " + invocation + " })(function() { "
                "const resource = new Map; return {" + ", ".join(fields) + "}; });",
                self.setup,
            ]
        )
        index = 0
...
def cases():
    ...
    result = [
        ...
        Case(
            "entry_alias",
            [
                statement("alias = objectA;"),
                store,
                statement("alias.value = 22;"),
                call("host.slot.value('a')", 22),
                call("host.slot.read('a')", OBJECT, "objectA"),
        ...
        Case(
            ...
            [
                store,
                call("alias = host.slot.read('a')", OBJECT, "objectA"),
                observe("alias === objectA", True),
                statement("alias.value = 23;"),
                call("host.slot.value('a')", 23),
                observe("objectA.value", 23),
                call("host.slot.read('a')", OBJECT, "objectA"),
            ],
        ),
        Case(
        ...
        Case(
            ...
            [
                store,
                call("host.slot.store('b', objectA)", 0),
                call("host.slot.equal('a', 'b')", True),
                call("host.slot.replace('a', 24)", 24),
                call("host.slot.value('b')", 24),
                observe("objectA.value", 24),
            ],
        ),
        Case(
            "object_key_and_value",
            [
                call("host.slot.store(objectA, objectB)", 0),
        ...
        Case(
            "self_key_value",
            [
                call("host.slot.store(objectA, objectA)", 0),
        ...
        Case(
            ...
            [
                store,
                statement("objectA.value = 'changed';"),
        ...
        Case(
            "nested_retention",
            [
                call("host.slot.nest(keyA, objectA)", 0),
                ...
                call("host.slot.peek(keyA)", 32),
                call("host.slot.peek(keyB)", 11),
                call("host.slot.nest(keyA, objectB)", 0),
                call("host.slot.payload(keyA)", OBJECT, "objectB"),
                call("host.slot.peek(keyA)", 11),
            ],
            methods=NESTED,
        ),
        Case(
            ...
            [
                ...
                statement("alias.value = 33;"),
                call("host.slot.peek(keyB)", 33),
                call("host.slot.payload(keyB)", OBJECT, "objectA"),
            ],
            methods=NESTED,
        ),
        Case(
        ...
        Case(
            "dynamic_field",
            [store, call("host.slot.dynamic('a', 'value')", 11)],
        ...
        Case(
            "non_scalar_field",
            [store],
            first_object="{value: 11, child: {value: 1}}",
            completed=0,
            boundary="store",
        ),
        ...
        Case(
            "accessor",
            [store],
            first_object="{get value() { return 11; }}",
            completed=0,
            calls=0,
            boundary=None,
        ),
        Case(
            "prototype",
            [store],
            setup="objectA.__proto__ = {value: 31};",
            completed=0,
            calls=0,
            boundary=None,
        ),
        Case(
            "host_payload", [call("host.slot.store('a', host)", 0)], completed=0, boundary="store"
        ),
        Case(
            "table_payload",
            [call("host.slot.store('a', host.slot)", 0)],
            completed=0,
            boundary="store",
        ),
        Case(
            "non_scalar_replacement",
            [store, statement("objectA.value = objectB;"), call("host.slot.value('a')", OBJECT)],
            completed=1,
            calls=2,
            boundary=None,
        ),
        Case(
            "published_payload",
            [store, statement("host.extra = objectA;"), call("host.slot.value('a')", 11)],
            completed=1,
            calls=2,
            boundary=None,
        ),
    ...
    refused = {
        ...
        "object_object_cycle": (
            "const payload = resource.get(key); payload.self = payload; return 0;",
            0,
            None,
        ),
        "provider_local_object": (
            "const payload = {value: 5}; resource.set(key, payload); return 0;",
            0,
            None,
        ),
        ...
        "unknown_after_writes": (
            "resource.set('tentative', value); resource.get(key).value = 42; "
            "globalThis.foreign(); return 0;",
            0,
            "TypeError",
        ),
        "throw_after_writes": (
            "resource.set('tentative', value); resource.get(key).value = 43; " "throw 7;",
            0,
            "number:7",
        ),
    }
    ...
    combined = {
        "combined": (
            "key, value",
            "resource.set(key, value); value.value = 44; "
    ...
    result.extend(
        [
            Case(
                ...
                [
                    call("host.slot.combined('a', objectA)", 44),
                    call("host.slot.read('a')", OBJECT, "objectA"),
                    call("host.slot.value('a')", 44),
                    observe("objectA.value", 44),
                ],
                methods=combined,
                callback=callback,
                final_count=1,
            ...
            Case(
                "callback_throw_after_writes",
                [call("host.slot.combined('a', objectA)", 0)],
                methods=combined,
                callback=callback + " throw 7;",
                final_count=1,
                completed=0,
                boundary="combined",
                error="number:7",
...
def oracle(node, folder, case, js):
    ...
    request.write_text(
        json.dumps({"source": str(js.resolve()), "observations": list(case.expected())})
    )
    ...
    if result["error"] != case.error or result["observations"] != expected:
        raise RuntimeError(
            f"{case.name}: Node returned {result}, expected {case.error}, {expected}"
        )
...
def contract(opt, ir):
    ...
    return dict(
        ...
        observations=observations + ["callCount"],
...
def check_report(case, report, before, after):
    ...
    for row, step in zip(rows, case.provider_steps()):
        ...
        if step.identity:
            ...
            if any(
                name != step.identity and value == object_id for name, value in identities.items()
            ):
            ...
            raise RuntimeError(
                f"{case.name}: {step.expression} has the wrong summary result: {row}"
            )
        ...
        for operation in row["object_operations"]:
            ...
            if object_id <= 0 or min(origin) < 0:
                raise RuntimeError(
                    f"{case.name}: object lost allocation/invocation provenance: {operation}"
                )
            if object_id in provenance and provenance[object_id] != origin:
                raise RuntimeError(
                    f"{case.name}: live object changed allocation/invocation provenance"
                )
        ...
        if method in {"value", "label", "peek", "replace"}:
            ...
            if not reads or mutations.attribute(reads[-1]["result"]) != mutations.primitive(
                step.expected
            ):
        ...
        if method in {"replace", "removeField", "combined"}:
            ...
            if not writes:
                raise RuntimeError(
                    f"{case.name}: completed object effect has no source proof: {row}"
                )
    ...
    if report["provider_reads"] or report["selected_branches"]:
        raise RuntimeError(
            f"{case.name}: object following reused old empty state or erased observers"
        )
...
def controls(args, prepared):
    ...
    legacy, _, _ = mutations.specialize(
        args.opt, original, manifest, args.work / "legacy", options=mutations.OPTIONS
    )
    ...
    for dependency in (
        "follow-publication=true",
        "follow-provider-reads=true",
        "follow-provider-mutations=true",
    ):
        report, _, _ = mutations.specialize(
            args.opt,
            original,
            manifest,
            args.work / ("without-" + dependency.split("=")[0]),
            options=OPTIONS.replace(dependency, ""),
            success=False,
        )
        no_facts(report, dependency)
    no_map, _, _ = mutations.specialize(
        args.opt,
        original,
    ...
    _, rewritten, _ = mutations.specialize(
        args.opt, original, manifest, args.work / "once", options=OPTIONS
    )
    stale, _, _ = mutations.specialize(
        args.opt, rewritten, manifest, args.work / "stale", options=OPTIONS, success=False
    )
    no_facts(stale, "stale source")
    rerun, _, _ = mutations.specialize(
    ...
    forged.write_text(
        unsafe.read_text().replace(
            ...
            "ctnative.host_object_identity = 99 : i64, ctnative.host_object_fields = true, ",
            1,
        )
    )
    report, _, _ = mutations.specialize(
    ...
    for name in ("nested_retention", "combined_transaction"):
        ...
        budget_manifest, budget_report = (
            args.work / f"{name}-budget.json",
            args.work / f"{name}-budget.report.json",
        )
        ...
        def run_budget(limit):
            ...
            if not report["valid"]:
                ...
                raise RuntimeError(
                    f"{name}: complete work limit lost provider/object state: {report}"
                )
            return report["valid"]
...
def main():
    ...
    for case in selected:
        ...
        report, output, _ = mutations.specialize(
            args.opt, ir, contract(args.opt, ir), folder / "checked", options=case.options()
        )
    ...
    mode = (
        "Node source cases"
        if args.oracle_only
        else "source cases, live identities, own fields and rollback controls"
    )
