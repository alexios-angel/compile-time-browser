...
def main():
    ...
    sources = {
        ...
        "alias_replaced": source(
            middle="alias = host.slot; alias.get = function replacement() { return 42; };"
        ),
        "table_replaced": source(
            middle="host.slot = {get: function replacement() { return 42; }};"
        ),
        "two_factories": source(twice=True),
        "unknown_after": source(middle="globalThis.foreign();"),
        "accessor_after": source(
            middle="Object.defineProperty(host.slot, 'get', {get: function() { return function() { return 0; }; }});"
        ),
        "unknown_factory": source(
            "function() { const resource = new Map; globalThis.foreign(); return {get: () => resource.size}; }"
        ),
        "writable_capture": source(
            "function() { let resource = new Map; return {get: () => { resource = new Map; return 42; }}; }"
        ),
        "resource_escape": source(
            "function() { const resource = new Map; return {resource: resource, get: () => resource.size}; }"
        ),
        "forwarded_capture": source(
            "function() { const resource = new Map; return {get: () => () => resource.size}; }"
        ),
        "post_capture_overwrite": source(
            "function() { let resource = new Map; const table = {get: () => resource.size}; resource = 0; return table; }"
        ),
    ...
    for name, ir in prepared.items():
        contract = dict(
            host.manifest(args.opt, ir), initial_intrinsics=["Map"], realm_global_this=True
        )
        report, output, result = prefix.specialize(
            args.opt,
            ir,
            contract,
            args.work / f"{name}-follow",
            options="follow-publication=true",
            success=name != "provider_replaced",
        )
        ...
        if name in positive:
            ...
            if name == "two_factories" and (
                len(report["factories"]) != 2 or report["targets"][-1].startswith("firstOnly$")
            ):
            ...
            if report["summarized_factories"] != 1 or report["resolved_calls"] != 1:
                raise RuntimeError(
                    "unknown intervening effect did not stop current-value discovery"
                )
        elif (
            report["summarized_factories"]
    ...
    report, rewritten, _ = prefix.specialize(
        args.opt, original, contract, args.work / "once", options="follow-publication=true"
    )
    ...
    for limit in (0, 200):
        limited, _, _ = prefix.specialize(
            ...
            options=f"follow-publication=true max-steps={limit}",
            success=False,
        )
        if (
            limited["valid"]
            or limited["resolved_calls"]
            or limited["summarized_factories"]
            or limited["capture_edges"]
        ):
    ...
    forged.write_text(
        unsafe.read_text().replace(
            "module attributes {",
            "module attributes {ctnative.host_factory_proved = true, ctnative.host_capture_edges = 1 : i64, ",
            1,
        )
    )
    ...
    print(
        "host publication: exact factories, current method/table replacements, distinct invocations and ownership/effect guards passed"
    )
