...
// RUN: ctjs-translate --ctbrowser-js-to-ctjs %S/Specialization/specialization-dispatch.js | ctjs-opt --ctjs-resolve-globals --ctjs-lift-to-scf --ctnative-specialize --ctnative-partial-evaluate | FileCheck %s --check-prefix=DISPATCH
// RUN: ctjs-translate --ctbrowser-js-to-ctjs %S/Specialization/specialization-dispatch.js | ctjs-opt --ctjs-resolve-globals --ctjs-lift-to-scf --ctnative-specialize --ctnative-partial-evaluate --ctnative-lower-to-emitc | FileCheck %s --check-prefix=NATIVE --implicit-check-not=ctnative.not_native --implicit-check-not=ctjs.func
