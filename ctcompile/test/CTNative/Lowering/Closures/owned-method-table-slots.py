...
def refused(args, prepared, name, *, imported=None):
    ...
    if name == "mutable_capture":
        expected = (
            "returned closure capture 0 is a mutable or late-initialized binding; "
        ...
        "method table" in reason or "method field" in reason or "owned method-table" in reason
...
def refusal_sources(factory):
    ...
    sources["escaped_owner"] = (
        factory + "function publish(seed) { const ns = {exports: makeData(seed)}; return ns; }\n"
    ...
    sources["table_scalar_schema"] = (
        factory + "function choose(seed) { if (seed > 0) { return makeData(seed); } return 0; }\n"
        "function publish(seed) { const ns = {exports: choose(seed)}; return ns.exports; }\n"
        + ordinary_run
    )
    sources["different_table_schemas"] = (
        factory
...
def main():
    ...
    for name, js, denominator, expected, fields in programs:
        ...
        if imported != denominator:
            raise RuntimeError(
                f"{name}: expected {denominator} source functions, imported {imported}"
            )
        oracle = run([str(reference), str(js)])
        if oracle.stdout != expected:
            raise RuntimeError(
                f"{name}: interpreter disagrees with fixture oracle\n{oracle.stdout}"
            )
        ...
        if (
            CTJS_FUNCTION.search(native)
            or REFUSAL.search(native)
            or len(NATIVE_FUNCTION.findall(native)) != denominator
        ):
        ...
        for mode, ir in [("explicit", module), ("deduced", deduced)]:
            ...
            for field in fields:
                if not re.search(
                    r"std::shared_ptr<ctnative::method_\w+>\s+" + field + r"\s*;", cpp
                ):
                    raise RuntimeError(
                        f"{name}/{mode}: {field} does not own its method-table carrier\n{cpp}"
                    )
            if name == "families":
                tables = set(
    ...
    print(
        f"owned method-table slots: native 6/6 specimen=4211, 5 lifetime observations, "
