...
def check(args, name, source, reason=None):
    ...
    result = subprocess.run(
        [args.opt, str(path), "-o", str(output)], text=True, capture_output=True, timeout=30
    )
    if reason is None:
...
def thrown(kind="f64"):
    ...
    return (
        f'    %value = emitc.literal "{value}" : {kind}\n'
...
def chain(size):
    pieces = [
        helper(f"helper{index}", f"    emitc.call @helper{index + 1}() : () -> ()\n")
...
def main():
    ...
    run(
        "transitive-mismatch",
        module(helper("leaf", thrown("i1")), middle, guard("middle")),
        MISMATCH,
    )
    ...
    run(
        "recursive",
        module(helper("leaf", "    emitc.call @leaf() : () -> ()\n"), guard()),
        RECURSION,
    )
    run(
        "mutual-recursion",
        module(helper("leaf", "    emitc.call @middle() : () -> ()\n"), middle, guard()),
        RECURSION,
    )
    ...
    marked = module(
        helper("leaf", thrown(), attributes=" attributes {ctnative.exception_payload = f64}"),
        guard(),
    )
    run("marked", marked)
    marker_output = (args.work / "marked.verified.mlir").read_text()
    changed, literal_count = re.subn(
        r'((?:emitc\.)?literal) "42\.0" : f64', r'\1 "true" : i1', marker_output
    )
    changed, throw_count = re.subn(r"(ctnative\.cpp_throw %[-\w.$]+) : f64", r"\1 : i1", changed)
    ...
    print(
        f"native call exception verifier: {accepted} accepted, "
