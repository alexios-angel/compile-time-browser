...
|---|---|---|
| Binary64: `0.1 + 0.2 !== 0.3`, `NaN !== NaN`, rounding near 2^53, signed zero | `js_num` retains binary64, NaN, infinities, signed zero and partial ordering; arithmetic and conversions carry the class. Number bitwise operators now share public Core ToInt32/ToUint32 with the VM, reduce modulo 2^32 and mask shift counts; 76 primitive observations pass in eight native modes. | Add source witnesses for rounding, subnormals, infinities, remainder and exponentiation edge cases; preserve results through folding, specialization and code generation. Keep BigInt bitwise operations separate; raw out-of-range C++ integer casts are not equivalent. Never use integer arithmetic merely because inputs are integral. |
