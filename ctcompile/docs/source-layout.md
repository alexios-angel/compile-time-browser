...
Refactor validation follows the focused policy in [CLAUDE.md](../../CLAUDE.md):
build affected targets on the devbox, run affected CTests/lit cases and the pinned
formatting check, and compare affected generated output. Check the registered
test-name set when moving tests. Full-suite replay requires an explicit user
request. No public pass name or generated-program interface changes with this
organization.
