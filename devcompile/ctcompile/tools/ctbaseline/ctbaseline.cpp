...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (ext == "html") {
            ...
            stages.push_back(stage{"html_parse_and_dom_build", ms, source.size(), nodes});
        } else if (ext == "css") {
            std::size_t rules = 0;
            ...
            stages.push_back(stage{"css_parse", parse, source.size(), rules});
            ...
            stages.push_back(stage{"css_parse_and_file", file_it, source.size(), filed});
        } else if (ext == "js") {
