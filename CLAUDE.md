...
| `docs/webgl-plan.md` | the GPU back end and the software rasteriser's GLSL |
| `docs/angle-plan.md` | **ANGLE as the WebGL back end** — stop implementing GLES and call the one Chrome ships. ~5,700 lines go, the 1,264-line JavaScript surface stays, and the whole of GLES 3.1 arrives at once. Three costs: ANGLE is GN + depot_tools with no CMake and is built with clang-cl on Windows where this tree uses llvm-mingw (**the top risk, and stage 0 exists to answer it**); the four WebGL goldens need ANGLE-over-Vulkan-over-lavapipe to stay byte-comparable; and every frame needs a `glReadPixels` back into the canvas bitmap |
