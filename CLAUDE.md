...
| `docs/webgl-plan.md` | the GPU back end and the software rasteriser's GLSL. **Includes the libshaderc verdict**: glslang refuses ES 1.00 and ES 3.00 for SPIR-V, which are exactly the two versions WebGL uses, so it cannot replace `raster/spirv.cpp` — measured, not assumed |
