...
//
// test/corpus/babylon/babylon_ratchet.cpp climbs a ladder and stops at the first failure,
// which tells you ONE thing. This runs everything and tells you the shape of the
// gap. Both are needed, and the reason is recorded in test/corpus/phaser/phaser-api.txt: the
...
// three months later. test/corpus/babylon/babylon-api.txt is what says which are which.

globalThis.__probes = [
    // --- the objects a scene is made of -------------------------------------
    ['mesh', 'CreateBox', function (scene) {
        var m = BABYLON.MeshBuilder.CreateBox('b', {
            size: 1
        }, scene);
        return 'verts=' + m.getTotalVertices();
    }],
    ['mesh', 'CreateSphere', function (scene) {
        var m = BABYLON.MeshBuilder.CreateSphere('s', {
            diameter: 1,
            segments: 8
        }, scene);
        return 'verts=' + m.getTotalVertices();
    ...
    ['mesh', 'CreateGround', function (scene) {
        var m = BABYLON.MeshBuilder.CreateGround('g', {
            width: 2,
            height: 2
        }, scene);
        return 'verts=' + m.getTotalVertices();
    ...
    ['mesh', 'CreateCylinder', function (scene) {
        var m = BABYLON.MeshBuilder.CreateCylinder('c', {
            height: 1,
            diameter: 1
        }, scene);
        return 'verts=' + m.getTotalVertices();
    ...
    ['mesh', 'CreateLines', function (scene) {
        var m = BABYLON.MeshBuilder.CreateLines('l', {
            points: [
                BABYLON.Vector3.Zero(), new BABYLON.Vector3(1, 1, 0)
            ]
        }, scene);
        if (!m) {
            throw 'no mesh';
    ...
    ['texture', 'DynamicTexture', function (scene) {
        var t = new BABYLON.DynamicTexture('dt', {
            width: 8,
            height: 8
        }, scene, false);
    ...
    ['texture', 'texture wrap and sampling modes', function (scene) {
        var t = new BABYLON.DynamicTexture('dt2', {
            width: 4,
            height: 4
        }, scene, false);
    ...
    ['scene', 'observables fire', function (scene) {
        ...
        scene.render();
        if (seen === 0) {
            throw 'onBeforeRenderObservable never fired';
        }
        return 'fired';
    }],
...
// is the same reason test/corpus/babylon/babylon_ratchet.cpp builds a page per rung.
