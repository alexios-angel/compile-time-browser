...
//
// test/corpus/webgl2/webgl2_ratchet.cpp asks whether Babylon draws AT ALL and is answered:
...
// recorded in test/corpus/babylon/babylon-ratchet.txt. The level may not go DOWN, and at the
// same level the blocker may not CHANGE, without this test failing. Only
// tools/corpus/ratchet.py babylon ratchet --advance writes the record, because a test that
...
[[nodiscard]] measurement measure(const std::string & bundle) {
    ...
    // after test/corpus/webgl2/webgl2_ratchet.cpp already proved the same scene lands.
    {
...
int main() {
    ...
    const std::string record = read_file("test/corpus/babylon/babylon-ratchet.txt");
    ...
    if (now.level > was) {
        std::printf("     AHEAD of the record (%d > %d) - run tools/corpus/ratchet.py babylon "
                    "ratchet --advance\n",
                    now.level, was);
        return 0;
    }
