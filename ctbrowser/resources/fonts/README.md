# resources/fonts/ — the default typefaces

(In a shipped application these travel as plain `fonts/` beside the exe, which
is still the runtime `font_path` default; in the source tree the suite and the
examples are pointed here with `CTBROWSER_FONT_PATH`.)
