...
`lib/App/app.cpp` are the only places that know SDL exists.
...
prefix, builds `ctbrowser/test/package/` against it via `find_package`.

`ctbrowser/test/lint/api_surface` lints the claim: application sources must contain exactly
...
reports, which is the whole point of having them, and `ctbrowser/test/corpus/phaser/phaser_ratchet.cpp`
...
`ctbrowser/test/golden/`, with `CTBROWSER_FONTS=font8x8` so the golden pins LAYOUT
...
found it. `ctbrowser/unittests/unit/data_url.cpp` asserts on the decoded bytes and on a real
...
nothing could see: `tests/` is SDL-free by an invariant `ctbrowser/test/lint/api_surface`
...
the same library. `ctbrowser/unittests/unit/data_url.cpp` asserts PNG against BMP pixel for pixel
...
consulted first. `ctbrowser/unittests/unit/net_basics` proves the client against a loopback
...
and it is why `ctbrowser/unittests/unit/image_basics` read a fetch's result before `load_html`
returned.
...
`loadTable` are p5's own code over `fetch`, and `ctbrowser/test/corpus/p5/p5_api.cpp` bakes three
