...
`ctbrowser/lib/Script/compile/`, and no consumer parses any of it.
...
`test/support/{tsan,lsan}.supp`, and both files say they were verified by planting
...
`test/golden/`, so every golden test would have failed on a fresh clone
...
(`/*.ppm`), so nothing reaches into `ctbrowser/test/golden/` to need an exception —
...
tools/check/compare.py start --engine=ctbrowse --engine=chrome \
                 --headed --delay 400 ctbrowser/examples/pages/widgets.html
...
**`tools/ctdrive/ctdrive.cpp` is the ctbrowser half**, and the two engine changes it
