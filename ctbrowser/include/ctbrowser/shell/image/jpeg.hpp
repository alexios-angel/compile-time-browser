...
// application layer fills in from SDL3_image. `test/` and `unittests/` are SDL-free by an
// invariant `test/lint/api_surface` lints for, so the entire suite saw a JPEG as a
...
// and net.hpp for curl.h. `lib/Shell/image/jpeg.cpp` is the only translation unit
