...
// and MathML's, which is `node_ns::other` with the URI recorded on the
// document (document::element_namespace): the HTML parser's second foreign
// vocabulary, and the one `<math>` puts an element in.
