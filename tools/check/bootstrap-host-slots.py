...
def main() -> None:
    ...
    if functions != expected_functions or "ctjs.skipped" in ir:
        raise RuntimeError(
            f"exact source accounting changed: {functions}, expected {expected_functions}"
        )
    result = run(
    ...
    binding, key = {
        ...
        "browser": ("globalThis", "bootstrap"),
        "amd": ("define", "amd"),
    }[args.mode]
    present = {"commonjs": {"module", "exports"}, "browser": {"globalThis"}, "amd": {"define"}}[
        args.mode
    ]
    ...
    if args.mode != "amd":
        ...
        if (
            ...
            or slot["source_writes"] < minimum_writes
            or slot["source_reads"] < 10
        ):
        ...
        raise RuntimeError(
            "retained AMD callable should retain its explicit unsupported owner frontier"
        )
    ...
    print(
        f"bootstrap host slots ({args.mode}): {functions} source functions, "
