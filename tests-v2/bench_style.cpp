// Style resolution: v2 against v1, on the same document and the same sheet.
//
...
// layout does not want one property, it wants all of them, so v1 is charged
// for the query() calls it would really make (one per property per element)
// and v2 for resolving each element once. Charging v1 for a single property
// would flatter it and measure nothing anybody runs.
...
// The properties a layout pass actually asks for. v1 pays query() per element
// per entry in this list; v2 resolves the element once and reads them out.
...
[[nodiscard]] std::string build_html(int sections, int rows) {
	...
	for (int s = 0; s < sections; ++s) {
		out += "<section class='panel s" + std::to_string(s % 8) + "'><header><h2>T</h2></header><ul class=list>";
...
[[nodiscard]] std::string build_css(int extra_rules) {
	...
	// Bulk rules that match nothing. v1 rescans them for every lookup; v2 files
	// them in buckets it never consults. That difference IS the point.
...
// Collect every element with its ancestor chain in v1's element_ref form,
// root-first and self-last, which is the order v1 documents.
//
...
// v1 then matches against freed memory. It looked like it worked and produced
...
void collect_v1(const read_txn & txn, atom_table & atoms, node_id at,
	...
	if (is_element) {
		path.push_back(owned_element{
		    std::string{atoms.text(txn.tag(at).value_or(atom{}))},
...
int main() {
	...
	// --- v1: query() per property per element -----------------------------
	std::size_t v1_hits = 0;
	...
	const auto v1_time = clock_type::now() - t0;

	// --- v2: resolve each element once ------------------------------------
	std::size_t v2_hits = 0;
	const auto t1 = clock_type::now();
	const style::style_map resolved = styles.resolve_all(txn);
	for (const auto & [key, style] : resolved) {
		for (const std::string_view property : wanted) {
			if (!style->get(atoms.intern_lower(property)).empty()) { ++v2_hits; }
		}
	}
	const auto v2_time = clock_type::now() - t1;

	const double v1_ms = std::chrono::duration<double, std::milli>(v1_time).count();
	const double v2_ms = std::chrono::duration<double, std::milli>(v2_time).count();

	std::printf("  v1  query() per property   %8.2f ms\n", v1_ms);
	std::printf("  v2  resolve once + read    %8.2f ms   %.1fx faster\n", v2_ms, v1_ms / v2_ms);
	std::printf("\n  declarations found: v1=%zu v2=%zu %s\n", v1_hits, v2_hits,
	            v1_hits == v2_hits ? "(agree)" : "(DISAGREE - the comparison is not like-for-like)");
