...
struct page {
	...
	void load(std::string_view html, std::string_view css, float viewport) {
		...
		const layout::engine eng{font8x8_advance};
...
void test_scrolling_recomposites_without_rastering() {
	...
	// and are still valid; v1 re-ran layout and re-emitted every command.
...
void test_a_fixed_layer_does_not_move() {
	...
	// scroll does not move. v1 carried a `fixed` bool on every paint command.
