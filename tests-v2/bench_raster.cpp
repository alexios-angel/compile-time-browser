...
//
// The second number is the one stage 5 exists to change. v1's engine::frame()
// re-runs layout and re-emits every paint command on every frame, so a scroll
// there costs a full layout - there is no cheaper path to measure it against,
// which is why v1's layout time IS its scroll time in the table below.
//
// v2 splits the frame into three, and only the last one runs on a scroll:
//
...
void run_case(int sections, int rows, int viewport_w, int viewport_h, scheduler & pool) {
	...
	const bench_v1::result v1 = bench_v1::layout(html, viewport_w, 10);
	...
	std::printf("%4d x %-4d %7zu %5zu/%-5zu  %8.3f %8.3f %8.3f %8.3f %8.3f  %7.1fx %5.1f\n",
	            sections, rows, placed.count(), after_first, all_tiles, v1.ms, record_ms, first_ms,
	            first_par_ms, scroll_ms, v1.ms / scroll_ms, tiles_per_scroll);
}
...
int main() {
	...
	std::printf("frame cost, 900x700 viewport, software backend, %zu pool workers\n\n",
	            pool.worker_count());
	std::printf("%-12s %7s %11s  %8s %8s %8s %8s %8s  %9s %s\n", "  document", "frags",
	            "tiles 1st/all", "v1 lay", "record", "raster", "ras par", "scroll", "v1/scroll",
	            "tiles/scroll");
	...
	std::printf("\nv1 lay is ctbrowser::layout(). v1 has no cheaper scroll path - engine::frame()\n"
	            "re-runs layout every frame - so that column IS its per-scroll cost.\n\n"
	            "v2's scroll column is a real scroll frame: raster whatever came into view,\n"
