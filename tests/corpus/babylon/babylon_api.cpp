...
// as a wrong answer. tests/corpus/babylon/babylon-api.txt is what says which are which.
...
int main() {
    const std::string probes = read_file("tests/corpus/babylon/babylon-api-probe.js");
    if (probes.empty()) {
        std::printf("FAIL tests/corpus/babylon/babylon-api-probe.js is missing\n");
    ...
    // --- the pawl ----------------------------------------------------------
    const std::string record = read_file("tests/corpus/babylon/babylon-api.txt");
    if (record.empty()) {
        std::printf(
            "FAIL tests/corpus/babylon/babylon-api.txt is missing - it is the recorded surface\n");
        ++ctbrowser_test_failures;
        REPORT("babylon_api");
    }
